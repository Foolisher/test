...
public class ${model}ServiceImplTest {
    ...
    public void testFindAllBy() throws Exception {
        ...
        // Should be successfully
        when(${ali}Dao.findAllBy(anyMap())).thenReturn(${ali}s);
        Response<List<${model}>> allByResp = ${ali}Service.findAllBy(ImmutableMap.<String, Object>
                of("id", 1l));
        Assert.assertTrue("The found records must be more than one",
                allByResp.getResult().size()>=1);

        // Should be interrupted
        when(${ali}Dao.findAllBy(anyMap())).thenThrow(new RuntimeException("Interruption"));
        allByResp = ${ali}Service.findAllBy(ImmutableMap.<String, Object>
                of("id", 1l));
        Assert.assertTrue("Should be interrupted", allByResp.getError()!=null);

        // Should be fast failed
        allByResp = ${ali}Service.findAllBy(null);
    ...
    <#assign mockParams><#list fds as fd>any${fd.javaType?cap_first}()<#if fd_has_next>, </#if></#list></#assign>
    @Test
    ...
    <#assign t><#if "${fd.javaType}"=="String">"${fd.comment}-1"</#if><#if "${fd.javaType}"=="Long">1l</#if><#if "${fd.javaType}"=="Integer">1</#if><#if "${fd.javaType}"=="Date">DateTime.now().toDate()</#if></#assign>
        ...
        Paging<${model}> ${ali}List = ${ali}Dao.findBy${mn}(${params}, offset, limit);
